CREATE  PROCEDURE `create_moh_female_maternal_aggregation`(IN _startDate DATE,IN _endDate DATE, IN _location VARCHAR(255),IN _defaultCutOff INT,IN _birthDateDivider INT, IN _mState varchar(50),IN _ageGroup varchar(15))
BEGIN
...
insert into moh_cohort_disaggregated(age_group, gender,tx_new,tx_curr,tx_curr_ipt,tx_curr_screened_tb,0A,1A,2A,3A,4A,5A,6A,7A,8A,9A,10A,11A,12A,13A,14A,15A,16A,17A,0P,1P,2P,3P,4P,4PP,4PA,9P,9PP,9PA,11P,11PP,11PA,12PP,12PA,14P,14PP,14PA,15P,15PP,15PA,16P,17P,17PP,17PA,non_standard)
SELECT	
	"All" as age_group, _ageGroup as gender,
    ...
    COUNT(IF(state = 'On antiretrovirals' and floor(datediff(_endDate,last_appt_date)) <=  _defaultCutOff and current_regimen = '1A: d4T / 3TC / NVP (previous 1L)', 1, NULL)) as 1A,
    ...
    COUNT(IF(state = 'On antiretrovirals' and floor(datediff(_endDate,last_appt_date)) <=  _defaultCutOff and current_regimen = '3A: d4T / 3TC + EFV (previous EFV)', 1, NULL)) as 3A,
...
COUNT(IF(state = 'On antiretrovirals' and floor(datediff(_endDate,last_appt_date)) <=  _defaultCutOff and current_regimen = '1P: d4T / 3TC / NVP' , 1, NULL)) as 1P,
...
COUNT(IF(state = 'On antiretrovirals' and floor(datediff(_endDate,last_appt_date)) <=  _defaultCutOff and current_regimen = '3P: d4T / 3TC + EFV' , 1, NULL)) as 3P,
COUNT(IF(state = 'On antiretrovirals' and floor(datediff(_endDate,last_appt_date)) <=  _defaultCutOff and current_regimen = "4P: AZT / 3TC + EFV" , 1, NULL)) as 4P,
...
COUNT(IF(state = 'On antiretrovirals' and floor(datediff(_endDate,last_appt_date)) <=  _defaultCutOff and current_regimen = "14P: AZT / 3TC + DTG", 1, NULL)) as 14P,
...
COUNT(IF(state = 'On antiretrovirals' and floor(datediff(_endDate,last_appt_date)) <=  _defaultCutOff and current_regimen = "17P: ABC / 3TC + EFV", 1, NULL)) as 17P,

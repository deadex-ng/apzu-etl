CREATE PROCEDURE `create_chronic_care_outcome`(IN _endDate DATE)
BEGIN
    ...
    CREATE TEMPORARY TABLE chronic_care_last_facility_outcome(
	 ...
	 INSERT INTO  chronic_care_last_facility_outcome(index_desc, pat, state,program,program_state_id, start_date,end_date,location)
...
FROM (SELECT
      ...
      FROM omrs_program_state,
                    (SELECT @r:= 1) AS r,
                    (SELECT @u:= 0) AS u
                    where program = "CHRONIC CARE PROGRAM"
